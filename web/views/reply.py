...
def add_reply(pid):
    ...
    if name == "":
        return redirect("/posts/post/%d" % int(pid), error=u"请填入名字")
    if email == "":
        return redirect("/posts/post/%d" % int(pid), error=u"请填入邮箱地址")
    if origin_content == "":
        return redirect("/posts/post/%d" % int(pid), error=u"请输入评论内容")
    ...
    return redirect("/posts/post/%d" % (int(pid)))
